...
## Allowed Characters
The data may only contain the following characters:
...
- Printable Latin-1 supplement characters: 0xA1 (¡) to 0xAC (¬) and 0xAE (®) 0xFF (ÿ).
...
### Type Labels
Type labels can placed before a value, providing information on how to deserialize it. Type labels must be enclosed with `()` parentheses and may not contain whitespace. The top-level element is always required to have a type label, unless it is `null`.
