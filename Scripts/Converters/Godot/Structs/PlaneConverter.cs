...
namespace Rusty.Serialization.Gd
{
    ...
    public class PlaneConverter : VectorConverter<Plane, float>
    {
        ...
        protected override void SetElementAt(ref Plane vector, int index, ref float element)
        {
            switch (index)
            {
                case 0: vector.Normal = new(element, vector.Normal.Y, vector.Normal.Z); break;
                case 1: vector.Normal = new(vector.Normal.X, element, vector.Normal.Z); break;
                case 2: vector.Normal = new(vector.Normal.X, vector.Normal.Y, element); break;
