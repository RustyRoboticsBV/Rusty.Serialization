using System;
using Rusty.Serialization.Core.Nodes;
using Rusty.Serialization.Core.Serializers;

namespace Rusty.Serialization.CSCD
{
    /// <summary>
    /// A CSCD currency serializer.
    /// </summary>
    public class DecimalSerializer : Serializer<DecimalNode>
    {
        /* Public methods. */
        public override string Serialize(DecimalNode node, ISerializerScheme scheme)
        {
            // Parse raw decimal.
            string text = node.Value;

            // Handle 0.
            if (text == "0")
                return "$";
            if (text == "-0")
                return "-$";

            // Remove leading zeroes.
            while (text.StartsWith("-0"))
            {
                text = '-' + text.Substring(2);
            }
            while (text.StartsWith('0'))
            {
                text = text.Substring(1);
            }

            return '$' + text;
        }

        public override DecimalNode Parse(string text, ISerializerScheme scheme)
        {
            // Remove whitespaces.
            string trimmed = text?.Trim();

            try
            {
                // Empty strings are not allowed.
                if (string.IsNullOrEmpty(trimmed))
                    throw new ArgumentException("Empty string.");

                // Ensure dollar sign.
                if (!text.StartsWith('$') && !text.StartsWith("-$"))
                    throw new FormatException("Missing $");

                // Handle $ and -$ strings.
                if (trimmed == "$")
                    return new DecimalNode(0m);
                if (trimmed == "-$")
                    return new DecimalNode(-0m);

                // Get contents.
                string contents = null;
                if (text.StartsWith('$'))
                    contents = text.Substring(1);
                else if (text.StartsWith("-$"))
                    contents = text.Substring(2);

                // Check syntax.
                bool foundDot = false;
                for (int i = 0; i < contents.Length; i++)
                {
                    if (contents[i] == '.')
                    {
                        if (!foundDot)
                            foundDot = true;
                        else
                            throw new ArgumentException("Multiple decimal points.");
                    }
                    else if (contents[i] < '0' || contents[i] > '9')
                        throw new ArgumentException($"Illegal character '{contents[i]}' at {i}.");
                }

                // Parse.
                return new(contents);
            }
            catch (Exception ex)
            {
                throw new ArgumentException($"Could not parse string '{text}' as a decimal:\n{ex.Message}");
            }
        }
    }
}