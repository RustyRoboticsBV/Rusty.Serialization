...
namespace Rusty.Serialization.Serializers.XML
{
    ...
    public class TimeSerializer : Serializer<TimeNode>
    {
        ...
        public override string Serialize(TimeNode node, ISerializerScheme scheme)
        {
            ...
            return XmlUtility.Pack(serialized, "time");
        }
        ...
        public override TimeNode Parse(string text, ISerializerScheme scheme)
        {
            ...
            try
            {
                ...
                // Unpack XML.
                string contents = XmlUtility.Unpack(trimmed, "time");

                // Check negative sign.
                bool negative = contents.StartsWith('-');
                ...
                for (int i = negative ? 1 : 0; i < contents.Length; i++)
                {
                    switch (contents[i])
                    {
                        case 'Y':
                            if (year == null)
                                year = Parse(contents, ref i);
                            else
                                throw new ArgumentException("Duplicate Y.");
                            break;
                        case 'M':
                            if (month == null)
                                month = Parse(contents, ref i);
                            else
                                throw new ArgumentException("Duplicate M.");
                            break;
                        case 'D':
                            if (day == null)
                                day = Parse(contents, ref i);
                            else
                                throw new ArgumentException("Duplicate D.");
                            break;
                        case 'h':
                            if (hour == null)
                                hour = Parse(contents, ref i);
                            else
                                throw new ArgumentException("Duplicate h.");
                            break;
                        case 'm':
                            if (minute == null)
                                minute = Parse(contents, ref i);
                            else
                                throw new ArgumentException("Duplicate m.");
                            break;
                        case 's':
                            if (second == null)
                                second = Parse(contents, ref i);
                            else
                                throw new ArgumentException("Duplicate s.");
                            break;
                        case 'f':
                            if (millisecond == null)
                                millisecond = Parse(contents, ref i);
                            else
                                throw new ArgumentException("Duplicate f.");
                            break;
                        default:
                            throw new ArgumentException($"Invalid term '{contents[i]}'.");
                    }
                }
